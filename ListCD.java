...
public class ListCD {
    ...
    private CD[] cdr;    // mảng đối tượng CD
    private int count;    // số phần tử thực tế đang chứa
    ...
    public void tangKT() {
        if (cdr.length == count) {                       // mảng đã đầy
            CD[] tam = new CD[cdr.length * 2];     // tạo mảng mới gấp đôi
            System.arraycopy(cdr, 0, tam, 0, count); // copy dữ liệu cũ
            cdr = tam;                                  // GÁN LẠI tham chiếu
        }
    }

    public boolean themCD(CD crom) {
        // Bước 1: kiểm tra trùng mã
        for (int i = 0; i < count; i++) {
            if (cdr[i].getMaCD().equalsIgnoreCase(crom.getMaCD())) {
                return false;   // đã tồn tại → không thêm
            }
        }
        // Bước 2: đảm bảo còn chỗ
        tangKT();
        // Bước 3: gán vào cuối mảng
        cdr[count] = crom;
        count++;
        return true;
    }
    ...
    public boolean capNhatCD(String maCD, float gia) {
        for (int i = 0; i < count; i++) {
            if (cdr[i].getMaCD().equalsIgnoreCase(maCD)) {
                cdr[i].setGiaThanh(gia);   // phải qua setter vì gia là private
                return true;
            }
        }
        return false;
